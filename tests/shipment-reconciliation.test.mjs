...
async function run() {
  console.log('--- New tracking discovered: one shipment planned, shipped email planned ---');
  {
    ...
    const plan = await planShipmentReconciliation({ order, printifyOrder, existingShipments: [], hasEmail: noEmailsSent() });
  ...
  console.log('\n--- Reconciliation discovers a shipment FIRST: plans exactly one email; a later webhook (simulated by hasEmail flipping true) must not resend ---');
  {
    const order = { fulfillment_status: 'in_production' };
    const printifyOrder = { status: 'in-production', shipments: [{ carrier: 'usps', number: 'TRACK-AAA', url: 'https://track/AAA' }] };
    const firstPoll = await planShipmentReconciliation({ order, printifyOrder, existingShipments: [], hasEmail: noEmailsSent() });
    ...
    const secondLook = await planShipmentReconciliation({
  ...
  console.log('\n--- Duplicate polling of the same state: no duplicate anything ---');
  {
    ...
    const plan1 = await planShipmentReconciliation({ order, printifyOrder, existingShipments, hasEmail });
    const plan2 = await planShipmentReconciliation({ order, printifyOrder, existingShipments, hasEmail });
  ...
  console.log('\n--- Repeated canceled polling: no duplicate alert, and no-op once D1 already reflects it ---');
  {
    ...
    const firstPoll = await planShipmentReconciliation({ order: { fulfillment_status: 'submitted_to_printify' }, printifyOrder, existingShipments: [], hasEmail: noEmailsSent() });
    ...
    const secondPoll = await planShipmentReconciliation({ order: { fulfillment_status: 'submitted_to_printify' }, printifyOrder, existingShipments: [], hasEmail: (t) => t === 'printify_canceled_alert' });
    ...
    const thirdPoll = await planShipmentReconciliation({ order: { fulfillment_status: 'printify_canceled' }, printifyOrder, existingShipments: [], hasEmail: (t) => t === 'printify_canceled_alert' });
  ...
  console.log('\n--- One of two shipments delivered: order not marked delivered yet ---');
  {
    ...
    const plan = await planShipmentReconciliation({ order, printifyOrder, existingShipments, hasEmail: (t) => t.startsWith('shipped_') });
