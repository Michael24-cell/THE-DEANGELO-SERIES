...
- [ ] **Register `/api/printify-webhook` with Printify** — Printify has no dashboard form for this (confirmed for this shop, `sales_channel: "custom_integration"`); webhooks are managed entirely via their REST API. Use `scripts/register-printify-webhook.mjs` (see its header comment) to create/update all 4 required topics (`order:sent-to-production`, `order:updated`, `order:shipment:created`, `order:shipment:delivered`), all pointed at `https://thedeangeloseries.com/api/printify-webhook?key=<PRINTIFY_WEBHOOK_SECRET>` — a secret WE generate ourselves (`openssl rand -hex 32`), not one Printify issues (confirmed their webhook endpoints never return one). Set the same value as `PRINTIFY_WEBHOOK_SECRET` in Cloudflare. (Confirmed absent from both Preview and Production as of this pass — until it's set, this endpoint 503s every request by design.)
